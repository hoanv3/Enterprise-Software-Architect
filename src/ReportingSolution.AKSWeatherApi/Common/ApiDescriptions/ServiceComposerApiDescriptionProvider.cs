...
{
    internal sealed class ServiceComposerApiDescriptionProvider : IApiDescriptionProvider
    {
        ...
        public void OnProvidersExecuting(ApiDescriptionProviderContext context)
        {
            ...
            {
                if (endpoint is RouteEndpoint routeEndpoint && endpoint.DisplayName == "api/products/{productId}")
                {
        ...
        private ApiDescription CreateApiDescription(RouteEndpoint routeEndpoint)
        {
            ...
            var verb = httpMethodMetadata?.HttpMethods.Count > 0 ? httpMethodMetadata.HttpMethods[0] : null;
